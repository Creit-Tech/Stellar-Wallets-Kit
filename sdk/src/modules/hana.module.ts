...
export const HANA_ID: string = "hana";
...
export class HanaModule implements ModuleInterface {
  ...
  isAvailable(): Promise<boolean> {
    return Promise.resolve(typeof window !== "undefined" && !!window.hanaWallet?.stellar);
  }

  async getAddress(): Promise<{ address: string }> {
    try {
      await this.runChecks();
      const address: string = await window.hanaWallet!.stellar!.getPublicKey();
      return { address };
    } catch (e) {
      throw parseError(e);
    }
  ...
  async signTransaction(
    ...
    try {
      await this.runChecks();
      const signedTxXdr: string = await window.hanaWallet!.stellar!.signTransaction({
        xdr,
  ...
  async signAuthEntry(
    ...
    try {
      await this.runChecks();
      const signedAuthEntry: string = await window.hanaWallet!.stellar!.signAuthEntry({
        xdr: authEntry,
        accountToSign: opts?.address,
      });
  ...
  async signMessage(
    ...
    try {
      await this.runChecks();
      const signedMessage: string = await window.hanaWallet!.stellar!.signMessage({
        message,
        accountToSign: opts?.address,
      });
