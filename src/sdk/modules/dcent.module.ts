...
export class DcentModule implements ModuleInterface {
  ...
  async isAvailable(): Promise<boolean> {
    // Detection sentinel injected by both the D'CENT extension and the mobile in-app browser.
    if (window.stellar?.provider !== "dcent" || !window.dcentStellarProvider) return false;

    try {
      const { isConnected } = await window.dcentStellarProvider.isConnected();
      return isConnected;
    } catch (e) {
      console.error(e);
      return false;
