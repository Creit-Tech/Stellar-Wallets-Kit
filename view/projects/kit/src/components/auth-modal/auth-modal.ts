...
@Component({
  ...
  template: `
    <section class="flex w-full">
      @if (explanation()) {
        <div class="w-6/12 p-4 bg-(--swk-background-secondary) rounded-l-(--swk-border-radius)">
      ...
      <div [style.width.%]="!explanation() ? 100 : 50"
        ...
        <h2 class="px-4 text-[1.25rem] font-bold text-(--swk-modal-title-color) mb-4">Connect a Wallet</h2>

        <ul class="w-full grid gap-2 px-2">
          @for (wallet of wallets(); track wallet.id) {
            <li class="px-2 py-2 cursor-pointer flex items-center bg-(--swk-wallet-item-background) hover:bg-(--swk-wallet-item-hover-background) hover:border-(--swk-wallet-item-hover-border) border-1 border-(--swk-wallet-item-border) rounded-(--swk-wallet-item-border-radius) duration-(--swk-wallet-item-transition) ease">
              <app-avatar class="mr-2"
                          size="sm"
                          [image]="wallet.icon"
                          [fallback]="(wallet.name | slice: 0:1) + '' + (wallet.name | slice: -1)">
              </app-avatar>
              <p class="text-(--swk-dark)">{{ wallet.name }}</p>
            </li>
          }
